...
export class Tree extends THREE.Group {
  ...
  updateLeavesColor(color) {
    const mat = this.leavesMesh.material;
    if (mat) {
      mat.color.set(color); // Mettez à jour la couleur du matériau
    }
  }
  ...
  #generateBranch(rng, origin, orientation, length, radius, level = 1) {
    ...
    for (let i = 0; i <= this.params.geometry.sections; i++) {
      ...
      sectionRadius *=
        1 - this.params.branch.taper * (i / this.params.geometry.sections);
      ...
      for (let j = 0; j < this.params.geometry.segments; j++) {
        ...
        if (i > 0 && i < this.params.geometry.sections) {
          angle += rng.random(
            this.params.geometry.randomization,
            -this.params.geometry.randomization
          );
        }
        ...
        if (i > 0 && i < this.params.geometry.sections) {
          segmentRadius *=
            1 +
            rng.random(
              this.params.geometry.radiusVariance,
              -this.params.geometry.radiusVariance
            );
        }
      ...
      let sectionLength =
        (length / this.params.geometry.sections) *
        (1 +
          rng.random(
            this.params.geometry.lengthVariance,
            -this.params.geometry.lengthVariance
          ));
      sectionLength *= Math.min(1.0, sectionLength * this.params.maturity);

      if (level > 1 && i < this.params.geometry.sections - 1) {
        sectionLength = Math.max(
          0,
          sectionLength * (this.params.maturity - 0.5) * 2.0
        );
      }
      ...
      const gnarliness =
        this.params.maturity *
        (this.params.branch.gnarliness +
          this.params.branch.gnarliness1_R / sectionRadius);
  ...
  #generateLeaf(rng, origin, orientation, rotate90 = false) {
    ...
    const v = [
      ...
      v.applyEuler(localRotation).applyEuler(orientation).add(origin)
    );
  ...
  #generateChildBranches(rng, level, sections, parentLength) {
    ...
    const minBranches =
      level === this.params.branch.levels
        ? this.params.leaves.minCount
        : this.params.branch.minChildren;
    const maxBranches =
      level === this.params.branch.levels
        ? this.params.leaves.maxCount
        : this.params.branch.maxChildren;
    const childBranchCount =
      Math.round(rng.random() * (maxBranches - minBranches)) + minBranches;

    // Calculate the separation angle between branches
    const branchSepAngle =
      this.params.branch.sweepAngle / (childBranchCount - 1);
    ...
    for (let i = 0; i < childBranchCount; i++) {
      ...
      if (i < childBranchCount - 1) {
        let startIndex =
          this.params.geometry.sections * this.params.branch.start;
        let endIndex = this.params.geometry.sections * this.params.branch.stop;
        sectionIndex = Math.floor(
          rng.random() * (endIndex - startIndex) + startIndex
        );
      } else {
      ...
      if (i < childBranchCount - 1) {
        const r1 = new THREE.Quaternion().setFromEuler(
          new THREE.Euler(
            (this.params.maturity * this.params.branch.sweepAngle) / 2,
            0,
            0
          )
        );
        const r2 = new THREE.Quaternion().setFromEuler(
          new THREE.Euler(0, offset + i * branchSepAngle, 0)
        );
        ...
        section.orientation = new THREE.Euler().setFromQuaternion(
          r3.multiply(r2.multiply(r1))
        );
      ...
      let childBranchLength =
        ...
        (this.params.branch.lengthMultiplier +
          rng.random(
            this.params.branch.lengthVariance,
            -this.params.branch.lengthVariance
          ));

      if (level === this.params.branch.levels) {
        this.#generateLeaf(rng, section.origin, section.orientation.clone());
